...
cfg_dip = {"model_inp": "fbp", # "fbp", "random"
       ...
       "random_seed": 1, 
       "random_seed_noise": 2,
       "lambda_denoise" : 0.001,  # You can tune this value
       "num_steps": 1500,
       "N_noise": 4,  # Number of noise realisations for expectation
       "lr": 1e-4,
       "psnr_tv": 26.88} 

save_dir = "dip_results/self_guidance"
os.makedirs(save_dir, exist_ok=True)

save_dir_img = "dip_results/self_guidance/imgs"
...
if cfg_dip["model_inp"] == "fbp":
    z_init = x_fbp.clone().detach()
else:
    g = torch.Generator()
    g.manual_seed(cfg_dip["random_seed_noise"])
    z_init = torch.randn(x.shape, generator=g)
...
optim_z = torch.optim.Adam([z], lr=1e-1)
...
exp_weight = .99
out_avg = torch.zeros_like(z_init)

for i in tqdm(range(cfg_dip["num_steps"])):
    optim_model.zero_grad()
    optim_z.zero_grad()

    z_expand = z.expand(cfg_dip["N_noise"], -1,- 1, -1)  # Expand z for N_noise
    eta = 0.5 * z.max().detach() * torch.rand_like(z_expand)  # Random noise for self-guidance
    noisy_z = z_expand + eta
    x_pred = model(noisy_z)
    x_pred_mean = x_pred.mean(dim=0).unsqueeze(0)  # Average over noise realisations
    data_loss = torch.sum((ray_trafo.trafo(x_pred_mean) - y_noise)**2/L**2)
    denoise_loss = torch.sum((z_expand - x_pred.to(device)) ** 2)/cfg_dip["N_noise"]
    loss = data_loss + cfg_dip["lambda_denoise"] * denoise_loss
    ...
    with torch.no_grad():
        out_avg = exp_weight * out_avg + (1 - exp_weight) * x_pred_mean.detach()
        psnr_list.append(peak_signal_noise_ratio(x[0,0,mask].cpu().numpy(), out_avg[0,0,mask].detach().cpu().numpy(), data_range=x[0,0,mask].cpu().numpy().max()))
    
        if psnr_list[-1] > best_psnr:
            best_psnr = psnr_list[-1]
            best_psnr_idx = i 
            best_psnr_image = torch.clone(out_avg.detach().cpu()).numpy()
        
        if (i + 1) == 200:
            psnr_at_1000 = psnr_list[-1]
            reco_at_1000 = torch.clone(out_avg.detach().cpu()).numpy()
        ...
        if (i+1) % 50 == 0:
            ...
            im = axes[0,2].imshow(x_pred_mean[0,0,:,:].detach().cpu().numpy(), cmap="gray")
            fig.colorbar(im, ax=axes[0,2], fraction=0.08)
            axes[0,2].set_title("Reco")
            axes[0,2].axis("off")

            im = axes[1,0].imshow(out_avg[0,0,:,:].detach().cpu().numpy(), cmap="gray")
            fig.colorbar(im, ax=axes[1,0], fraction=0.08)
            axes[1,0].set_title("Reco (Avg)")
            axes[1,0].axis("off")
            ...
            axes[1,2].imshow(z[0,0,:,:].detach().cpu().numpy(), cmap="gray")
...
img = x_pred_mean.detach().cpu().numpy()[0,0] * 255
img = img.astype(np.uint8)
