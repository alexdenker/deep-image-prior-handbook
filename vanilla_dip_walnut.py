...
for i in tqdm(range(cfg_dip["num_steps"])):
    ...
    x_pred = model(z + noise_std**2*torch.randn_like(z))

    loss = torch.sum((ray_trafo.trafo(x_pred) - y_noise)**2/L**2) #+ 1e-5 * tv_loss(x_pred)
    loss.backward()
    ...
    psnr_list.append(peak_signal_noise_ratio(x[0,0,mask].cpu().numpy(), x_pred[0,0,mask].detach().cpu().numpy(), data_range=x[0,0,mask].cpu().numpy().max()))
    ...
    if (i + 1) == 200:
        ...
        reco_at_1000 = torch.clone(x_pred.detach().cpu()).numpy()
    ...
    if (i+1) % 50 == 0:
        ...
        axes[0,1].set_title("Data Consistency ||Ax - y ||^2")

        im = axes[1,0].imshow(x_pred[0,0,:,:].detach().cpu().numpy(), cmap="gray")
