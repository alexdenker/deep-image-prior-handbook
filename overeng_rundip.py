...
def run_dip(args, logger=None, paths=None):
    ...
    model = get_unet_model(in_ch=1, out_ch=1, scales=model_nsp.scales,
                            skip=model_nsp.skip,
    ...
    if args.method == "vanilla":
        ...
        x_pred, psnr_list, loss_list = dip.train(ray_trafo, y, z, x_gt=x, logger=logger,
                            ...
                            exp_weight=args.exp_weight, lr_z=1e-2)    
    else:
    ...
    ax2.set(xscale='log', yscale='log', title='Loss (log)', xlabel='Step', ylabel='Loss')
    ax2.grid(True)
...
if __name__ == "__main__":
    try:
        ...
        wandb_project = f"DIP_{args.method}_{args.model_inp}"
        wandb_entity = "zkereta"
        base_path = f"dip_results/{args.method}/{args.model_inp}" 
        paths = {
            "base": base_path,
        ...
        logger_kwargs = {
            ...
            "log_file": os.path.join(paths["logs"], f"log_{datetime.now():%Y-%m-%d_%H-%M-%S}.log"),
