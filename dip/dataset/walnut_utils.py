...
def get_first_proj_row_for_sub_sampling(
    factor=DEFAULT_PROJ_ROW_SUB_SAMPLING, num_orig=PROJS_ROWS, num=-1
):
...
def sub_sample_proj(
    projs,
    factor_row=DEFAULT_PROJ_ROW_SUB_SAMPLING,
    first_row=-1,
    num_rows=-1,
    factor_col=DEFAULT_PROJ_COL_SUB_SAMPLING,
    first_col=-1,
):
    if first_row == -1:
        first_row = get_first_proj_row_for_sub_sampling(
            factor=factor_row, num_orig=projs.shape[0]
        )
    max_num_proj_rows = len(range(first_row, PROJS_ROWS, factor_row))
    if num_rows == -1:
    ...
    out = projs[
        first_row : (first_row + num_rows * factor_row) : factor_row,
        :,
        first_col::factor_col,
    ]
    return out
...
def up_sample_proj(
    ...
    if factor_row != 1:
        if first_row == -1:
            ...
            first_row = get_first_proj_row_for_sub_sampling(
                factor=factor_row, num_orig=num_rows_orig
            )
...
def get_down_sampled_vol_shape(down_sampling=DEFAULT_VOL_DOWN_SAMPLING):
    ...
    down_sampled_vol_shape = np.floor(np.asarray(VOL_SZ) / down_sampling).astype(int)
...
def down_sample_vol(
    ...
    down_sampled_vol_shape = np.asarray(
        get_down_sampled_vol_shape(down_sampling=down_sampling)
    )
    ...
    if kind == "mean_pooling":
        ...
        raise NotImplementedError(f"Unknown down-sampling method '{kind}'")
...
def get_vol_geom(down_sampling=DEFAULT_VOL_DOWN_SAMPLING, num_slices=-1):
    ...
    vol_geom["option"]["WindowMinX"] = (
        vol_geom["option"]["WindowMinX"] * VOX_SZ * down_sampling[2]
    )
    vol_geom["option"]["WindowMaxX"] = (
        vol_geom["option"]["WindowMaxX"] * VOX_SZ * down_sampling[2]
    )
    vol_geom["option"]["WindowMinY"] = (
        vol_geom["option"]["WindowMinY"] * VOX_SZ * down_sampling[1]
    )
    vol_geom["option"]["WindowMaxY"] = (
        vol_geom["option"]["WindowMaxY"] * VOX_SZ * down_sampling[1]
    )
    vol_geom["option"]["WindowMinZ"] = (
        vol_geom["option"]["WindowMinZ"] * VOX_SZ * down_sampling[0]
    )
    vol_geom["option"]["WindowMaxZ"] = (
        vol_geom["option"]["WindowMaxZ"] * VOX_SZ * down_sampling[0]
    )
...
def get_proj_geom(
    ...
    data_path_full = os.path.join(
        data_path, f"Walnut{walnut_id}", "Projections", f"tubeV{orbit_id}"
    )
    ...
    max_num_proj_rows = len(range(first_proj_row, PROJS_ROWS, proj_row_sub_sampling))
    if num_proj_rows == -1:
        num_proj_rows = max_num_proj_rows
    else:
    ...
    num_proj_cols = len(range(first_proj_col, PROJS_COLS, proj_col_sub_sampling))
    ...
    proj_geom = astra.create_proj_geom("cone_vec", num_proj_rows, num_proj_cols, vecs)
...
def get_projection_data(
    ...
    projs = sub_sample_proj(
        ...
        first_row=first_proj_row,
        num_rows=num_proj_rows,
        factor_col=proj_col_sub_sampling,
        first_col=first_proj_col,
    )
...
def get_ground_truth_3d(data_path, walnut_id):
    gt_slices = [
        get_ground_truth(data_path, walnut_id, slice_ind)
...
def get_single_slice_ind(
    ...
    if single_slice_config is None:
        raise ValueError(
            "No single slice ray trafo configuration known for "
...
class WalnutRayTrafo:
    ...
    def __init__(
        ...
        self.vol_down_sampling = (
            (vol_down_sampling,) * 3
            if np.isscalar(vol_down_sampling)
            else vol_down_sampling
        )
        self.first_proj_row = (
            get_first_proj_row_for_sub_sampling(factor=self.proj_row_sub_sampling)
            if first_proj_row == -1
            else first_proj_row
        )
        self.first_proj_col = (
            get_first_proj_col_for_sub_sampling(factor=self.proj_col_sub_sampling)
            if first_proj_col == -1
            else first_proj_col
        )
        ...
        self.num_proj_rows = len(
            range(self.first_proj_row, PROJS_ROWS, self.proj_row_sub_sampling)
        )
        self.num_proj_cols = len(
            range(self.first_proj_col, PROJS_COLS, self.proj_col_sub_sampling)
        )

        self.vol_shape = get_down_sampled_vol_shape(
            down_sampling=self.vol_down_sampling
        )
        self.proj_shape = (self.num_proj_rows, self.num_angles, self.num_proj_cols)

        self.vol_geom = get_vol_geom(down_sampling=self.vol_down_sampling)

        proj_geom_kwargs = dict(
            data_path=self.data_path,
            walnut_id=self.walnut_id,
    ...
    def fp3d(self, vol_x, vol_geom=None):
        ...
        astra_fp3d_cuda(
            vol_x=vol_x, vol_geom=vol_geom, proj_geom=proj_geom, projs_out=projs
        )
    ...
    def bp3d(self, projs, proj_geom=None, proj_geom_no_sub_sampling=None):
        if self.proj_up_sampling_via_geom:
            ...
            proj_geom = (
                proj_geom_no_sub_sampling
                if proj_geom_no_sub_sampling is not None
        ...
        astra_bp3d_cuda(
            projs=projs, vol_geom=self.vol_geom, proj_geom=proj_geom, vol_x_out=vol_x
        )
...
class MaskedWalnutRayTrafo:
    ...
    def __init__(
        ...
        self.ray_trafo_full = WalnutRayTrafo(
            data_path,
            walnut_id,
            orbit_id,
            angular_sub_sampling=angular_sub_sampling,
    ...
    def build_proj_mask(self):
        if self.vol_mask_slice is None:
            ...
            vol_test[self.vol_mask_slice] = 1.0
            ...
            if self.proj_mask_select_k_rows is not None:
                ...
                vol_test_full[:] = 1.0
                projs_sum = self.fp3d(
                    vol_test_full, vol_geom=self.ray_trafo_full.vol_geom
                )
                ...
                for _ in range(self.proj_mask_select_k_rows):
                    index_array = np.expand_dims(np.argmax(fraction, axis=0), axis=0)
                    assert np.all(np.take_along_axis(projs, index_array, axis=0) > 0.0)
                    np.put_along_axis(proj_mask, index_array, True, axis=0)
                    np.put_along_axis(fraction, index_array, 0.0, axis=0)
            else:
                proj_mask[:] = projs > 0.0
            ...
            self.proj_mask_last_row_inds = (
                self.num_proj_rows - 1 - np.argmax(self.proj_mask[::-1], axis=0)
            )
    ...
    def get_inds_in_flat_projs_per_angle(self):
        ...
        for i in range(self.num_angles):
            ...
            inds_in_flat_projs_single_angle = inds_in_flat_projs_proj_array[
                single_angle_proj_mask
            ]
            single_angle_proj_mask[:, i, :] = 0.0
    ...
    def get_proj_slice_contributing_to_masked_vol(self):
        ...
        while i < self.num_proj_rows:
            ...
            if np.any(vol_x[self.vol_mask_slice] > 0.0):
                break
            i += 1
    ...
    def get_vol_slice_contributing_to_masked_projs(self):
        ...
        while i < self.num_slices:
            ...
            vol_test[i] = 1.0
            projs = self.fp3d(vol_test)
            if np.any(projs[self.proj_mask] > 0.0):
                break
            i += 1
        first_contributing = i
    ...
    def vol_from_vol_in_mask(self, vol_in_mask, out=None, padding_mode="edge"):
        vol_padded = np.zeros(self.vol_shape, dtype=np.float32) if out is None else out
    ...
    def projs_from_flat_projs_in_mask(
        self, flat_projs_in_mask, out=None, padding_mode="edge"
    ):
        assert self.proj_mask is not None

        projs_padded = (
            np.zeros(self.proj_shape, dtype=np.float32) if out is None else out
        )
        ...
        if padding_mode == "edge":
            if self.proj_mask_select_k_rows == 1:
                ...
                projs_padded[:] = np.take_along_axis(
                    projs_padded, self.proj_mask_first_row_inds[None], axis=0
                )
            else:
                ...
                for i in range(self.proj_shape[1]):
                    for j in range(self.proj_shape[2]):
                        ...
                        projs_padded[:first_ind, i, j] = projs_padded[first_ind, i, j]
    ...
    def apply(self, vol_in_mask, padding_mode="edge"):
        vol_x = self.vol_from_vol_in_mask(vol_in_mask, padding_mode=padding_mode)
    ...
    def apply_adjoint(self, flat_projs_in_mask, padding_mode="edge", squeeze=False):
        projs = self.projs_from_flat_projs_in_mask(
            flat_projs_in_mask, padding_mode=padding_mode
        )
...
def save_masked_ray_trafo_matrix(file_path, walnut_ray_trafo):
    ...
    assert walnut_ray_trafo.shift_z == 0.0

    vol_in_mask = np.zeros(VOL_SZ[1:])

    ray_trafo_matrix = scipy.sparse.dok_matrix(
        (walnut_ray_trafo.num_projs_in_mask, VOL_SZ[1] * VOL_SZ[2]), dtype=np.float32
    )
    for i in tqdm(range(VOL_SZ[1] * VOL_SZ[2])):
        vol_in_mask[:] = 0.0
        vol_in_mask[np.unravel_index(i, VOL_SZ[1:])] = 1.0

        flat_projs_in_mask = walnut_ray_trafo.apply(vol_in_mask)
        non_zero_mask = flat_projs_in_mask != 0.0
    ...
    scipy.io.savemat(
        file_path,
        {
            ...
            "vol_mask_slice": np.array(
                [  # matlab inds, first and last included
                    range(walnut_ray_trafo.num_slices)[walnut_ray_trafo.vol_mask_slice][
                        0
                    ]
                    + 1,
                    range(walnut_ray_trafo.num_slices)[walnut_ray_trafo.vol_mask_slice][
...
def get_masked_ray_trafo_matrix(file_path):
    matrix = scipy.io.loadmat(file_path, variable_names=["ray_trafo_matrix"])[
        "ray_trafo_matrix"
    ].astype("float32")
    return matrix
...
def save_single_slice_ray_trafo_matrix(
    ...
    save_masked_ray_trafo_matrix(os.path.join(output_path, filename), walnut_ray_trafo)
