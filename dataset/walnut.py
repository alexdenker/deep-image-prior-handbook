...
from .walnut_utils import (
    get_projection_data,
    get_single_slice_ray_trafo,
    get_single_slice_ind,
    get_ground_truth,
    VOL_SZ,
)


DEFAULT_WALNUT_SCALING_FACTOR = 14.0
...
def get_walnut_2d_observation(
    ...
    scaling_factor: float = DEFAULT_WALNUT_SCALING_FACTOR,
) -> Tensor:
    """
    ...
    observation_full = get_projection_data(data_path=data_path, **walnut_kwargs)

    # WalnutRayTrafo instance needed for selecting and masking the projections
    walnut_ray_trafo = get_single_slice_ray_trafo(data_path=data_path, **walnut_kwargs)
...
def get_walnut_2d_ground_truth(
    ...
    ground_truth = get_ground_truth(
        data_path=data_path, walnut_id=walnut_id, slice_ind=slice_ind
    )
...
def get_walnut_2d_inner_patch_indices(patch_size: int) -> List[int]:
    ...
    patch_idx_list = [
        patch_idx
        for patch_idx in range(num_patches_0 * num_patches_1)
        if patch_idx % num_patches_0 in range(num_patches_0)[patch_slice_0]
        and patch_idx // num_patches_0 in range(num_patches_1)[patch_slice_1]
    ]

    return patch_idx_list


def get_walnut_2d_inner_part_defined_by_patch_size(
    patch_size: int
) -> Tuple[slice, slice]:
    """
    ...
    slice_0 = slice(
        ...
        (
            patch_slice_0.stop * patch_size
            if patch_slice_0.stop < num_patches_0
            else VOL_SZ[1]
        ),
    )
    slice_1 = slice(
        ...
        (
            patch_slice_1.stop * patch_size
            if patch_slice_1.stop < num_patches_1
            else VOL_SZ[2]
        ),
    )
...
def get_walnut_data(config, ray_trafo):
    ...
    return torch.utils.data.TensorDataset(  # include batch dims
        noisy_observation[None, None],
        ground_truth[None, None],
        filtbackproj[None, None],
    )
